    ...
    public static class TogglSyncManager
    {
            ...
            IObservable<Unit> delayCancellation)
        {
            ...
            var persistWorkspaces =
                new PersistListState<IWorkspace, IDatabaseWorkspace, IThreadSafeWorkspace, WorkspaceDto>(dataSource.Workspaces, WorkspaceDto.Clean);
            ...
            var persistUser =
                new PersistSingletonState<IUser, IDatabaseUser, IThreadSafeUser, UserDto>(dataSource.User, UserDto.Clean);

            var noDefaultWorkspaceTrackingState = new NoDefaultWorkspaceTrackingState(analyticsService);

            var persistTags =
                new PersistListState<ITag, IDatabaseTag, IThreadSafeTag, TagDto>(dataSource.Tags, TagDto.Clean);

            var updateTagsSinceDate = new SinceDateUpdatingState<ITag, IDatabaseTag>(database.SinceParameters);

            var persistClients =
                new PersistListState<IClient, IDatabaseClient, IThreadSafeClient, ClientDto>(dataSource.Clients, ClientDto.Clean);
            ...
            var persistPreferences =
                new PersistSingletonState<IPreferences, IDatabasePreferences, IThreadSafePreferences, PreferencesDto>(dataSource.Preferences, PreferencesDto.Clean);

            var persistProjects =
                new PersistListState<IProject, IDatabaseProject, IThreadSafeProject, ProjectDto>(dataSource.Projects, ProjectDto.Clean);
            ...
            var persistTimeEntries =
                new PersistListState<ITimeEntry, IDatabaseTimeEntry, IThreadSafeTimeEntry, TimeEntryDto>(dataSource.TimeEntries, TimeEntryDto.Clean);

            var updateTimeEntriesSinceDate = new SinceDateUpdatingState<ITimeEntry, IDatabaseTimeEntry>(database.SinceParameters);

            var persistTasks =
                new PersistListState<ITask, IDatabaseTask, IThreadSafeTask, TaskDto>(dataSource.Tasks, TaskDto.Clean);
